...
{
  sendStringMsg = String(EncodingStrBuf);
...
void CommManagerLoop() 
{
	...
	{
		char c = ' ';      
		EncodingStrBufIndex = 0;
		while( client.available() )
		{
			c = client.read();
			if( c == ';' )
			{
				EncodingStrBuf[EncodingStrBufIndex] = 0;
				break;
			}
			EncodingStrBuf[EncodingStrBufIndex++] = c;
		}       

		DecodingJSONmsgToStructMsg(JsonBuffer, EncodingStrBuf);
		jTxMsg = jRxMsg;
		JsonObject& messageEncoding = EncodingStrucMsgToJSONmsg(JsonBuffer);     
		//messageEncoding.prettyPrintTo(Serial);   
		messageEncoding.printTo(EncodingStrBuf,sizeof(EncodingStrBuf)); 
		String sendStringMsg = EncodingStrBuf;
...
//  msg["send3"] = jTxMsg.send3.c_str();
	...
	jTxMsg.send1 = "arduino-send1";
	jTxMsg.send2 = "arduino-send1";
	jTxMsg.send3 = "arduino-send1";

	msg["send1"] = jTxMsg.send1;
	msg["send2"] = jTxMsg.send2;
	msg["send3"] = jTxMsg.send3;
